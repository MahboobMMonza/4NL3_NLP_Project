...
Despite these strong validation metrics, systematic error analysis identified a persistent vulnerability across all four implementations: a shared struggle to classify highly disguised threats that actively omit conventional spam keywords and flawlessly mimic legitimate conversational language. These structurally benign phishing vectors frequently bypassed the text-reliant classifiers, serving as the primary source of false negatives. 

Moving forward, we recommend that future research transitions away from relying strictly on explicit text representations to solve this ceiling effect. Instead, future work should focus on building hybrid deployment pipelines that utilize the strong semantic understanding of fine-tuned learning models alongside external, deterministic reputation-checking systems, such as automated URL cross-referencing and sender domain authentication networks. Additionally, exposing the models to heavily weighted adversarial phishing examples during the training phase could further fortify the classifiers against sophisticated evasion tactics.

\section*{Team Contributions}

Hamza implemented Model 1 (RoBERTa and TF-IDF Ensemble) and wrote the Introduction and Related Work sections. Omar built Model 2 (Random Forest Classifier) and wrote the Dataset section. Virochaan trained Model 3 (RoBERTa Base) and wrote the evaluation comparisons to other groups. Mahboob implemented Model 4 (ELECTRA Base), wrote the Conclusions section, and integrated the generic content formatting across the rest of the report.
