...
\subsection{Model 2: Random Forest Classifier}
The initial model consisting of just Random Forest achieved a validation accuracy of 95.6\% and a Codabench test score of approximately 95.4\%. Adding TF-IDF bigrams and additional input features raised the competition accuracy to 96.04\%, a 0.64 improvement. The model was subsequently tuned again with the addition of decision threshold tuning, and setting min\_samples\_leaf to 2. However, this resulted in a slight degradation in performance compared to the previous configuration. In a sparse feature space, this approach can hinder model sensitivity, and prevent the forest from splitting on rare but informative features. As a result, the model was again tuned to reach its final configuration by setting max\_features=500. This resulted in a test accuracy of 95.93\%. Although it was believed that this addition would increase the likelihood that informative features are considered at each node, the model appears to have reached a general bottleneck where further changes just increase complexity with no substantial improvement. Nontheless, this model still outperforms both Codabench baselines and requires no GPU resources, making it far lighter than the alternatives.
